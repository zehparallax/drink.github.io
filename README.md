...
icon-32/180/192/512.png App-Icons für Tab, iOS und Android
icon-maskable-*.png     Android-Varianten mit Sicherheitsrand
emoji-eins.png …        emoji-zehn.png — Platzhalter, einfach überschreiben
...
2. **Ein Icon direkt aufrufen:** `…/drink/icon-192.png`. Kommt kein Tropfen, fehlt die Datei im Repository.
...
| Datei | Zweck |
|---|---|
| `favicon.ico`, `favicon.svg`, `icon-32.png` | Browser-Tab und Lesezeichen |
| `icon-180.png` | iPhone und iPad; randlos, iOS rundet die Ecken selbst und verträgt keine Transparenz |
| `icon-192.png`, `icon-512.png` | Android- und Desktop-Installation, App-Umschalter, Splash-Screen |
| `icon-maskable-192.png`, `icon-maskable-512.png` | Android schneidet je nach Hersteller Kreis, Squircle oder Tropfen aus — deshalb 20 % Sicherheitsrand rundherum |

**Alle Dateien liegen flach im Wurzelverzeichnis, es gibt keinen Unterordner.** Das ist Absicht: Beim Hochladen über die GitHub-Weboberfläche gehen Unterordner je nach Browser und Vorgehen verloren, und ein fehlender Ordner `icons/` führt zu genau einem Symptom — die App installiert sich, hat aber kein Icon. Flach kann das nicht passieren.

Die Größe 32 steht bewusst **nicht** im Manifest, nur in den `<link rel="icon">`-Zeilen: Manche Launcher greifen sonst beim Installieren nach dem erstbesten Eintrag und legen ein winziges Icon auf den Startbildschirm.
